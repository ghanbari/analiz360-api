...
class AddSmsToOutboxAction extends AbstractController
{
    /**
    ...
    public function __invoke(Request $request, SmsMessage $data): array
    {
        ...
        if (empty($receptors) && empty($data->getUsers())) {
            $result = $manager->getRepository('App:SmsMessage')->sendMessageToAll($data);
